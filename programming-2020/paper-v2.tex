...
interactive process that blurs the traditional distinction between development-time and run-time.
This poses an interesting challenge for programming language research, inviting us to
revisit a number of traditional assumptions about programming languages and design a new kind of
programming tools that are suited for data analyst needs.

We present the \emph{data exploration calculus}, a formal language that captures the structure of
scripts written to explore data. We focus on the simplest kind of data analyses such as those
written by journalists to analyse open government data. We implement a programming environment,
based on this calculus, that evaluates code instantly during editing and shows previews of the results,
while allowing the user to modify code in an unrestricted way in a text editor. Supporting interactive
editing is tricky. Any edit can change the structure of code and fully recomputing the output would
be too expensive. We present a technique that allows evaluating and type-checking code while it is
being written and that reuses results of previous runs. We formalize the technique using the data
exploration calculus, prove that it is correct and specify when previous results are reused.
Finally, we illustrate the practicality of our approach using empirical evaluation
and a case study.
...
Increasing numbers of programming environments aim to provide a live development experience
for standard programming languages, but doing this is not easy. Fully recomputing the whole program
after every keystroke is inefficient. Calculating how a change in the source code changes the
...
\end{equation*}
%
A live coding environment computes and shows the list of years. Suppose that the programmer then modifies the code by
...
This can already be done in a number of systems based on structured code editors, but our
approach allows this behaviour using a standard text editor. We discuss related work in more detail in
Section~\ref{sec:future}, but we review the most important directions here, in order to
situate our contributions.

\subsection{Related work}
Simple data exploration performed, for example, by journalists \cite{ddj} is done either
programmatically or using spreadsheets. The former is reproducible and transparent, but requires
expert programming skills. The latter is easy, but it limits reproducibility and is error-prone.
We aim to bring some aspects of spreadsheets, especially liveness, to programmatic data exploration.
Recomputation in spreadsheets is well studied,~e.g. by \citet{spreadsheet}, but extending it to
code written and edited in an ordinary text editor poses interesting challenges.
...
by e.g.~\citet{dataflow,noworkflow} and \citet{wrattler}.
...
can is modified via known operations with known effect on the computation graph
...
%
The technical aspects of our technique for providing instant feedback
...
as noted in Section~\ref{sec:future}, a language that supports incremental computation could be used
...
\item Implementing a live coding environment requires a different way of thinking than the one
  presented in classic compiler literature, because it does not operate in a batch mode. We capture
  the essence of the new perspective (Section~\ref{sec:formal}) and use it to build our \emph{live
  preview} mechanism (Section~\ref{sec:previews}) that evaluates code instantly during editing.
...
\item We describe two extensions of our core environment. First, we add instant type checker
  that relies on the same core infrastructure as live previews and enables richer user experience
  through \emph{type providers} (Section~\ref{sec:extra-types}). Second, we add a code reuse mechanism
  that supports the live coding experience (Section~\ref{sec:extra-abstraction}).
\end{itemize}
...
engineers work. The different nature of data science workflow has been documented
by \citet{workflow}. This paper focuses on simple data wrangling and data exploration as done, for
example, by journalists analysing government datasets. This section illustrates how such data
analyses typically look and provides a justification for the design of our data exploration calculus.
...
initial focus is on the simplest use cases. We believe that making programmatic
data exploration more interactive and spreadsheet-like can encourage users to choose programming tools
over spreadsheets, allowing them to produce reproducible and transparent data analyses.

As an example, the analysis done by the Financial Times for an article on plastic waste
...
an extract from one notebook of the data analysis. The code has a number of important properties:
%
\begin{itemize}[itemsep=3pt]
...
\item There are many corner cases, such as the fact that the \ident{keep\_default\_na} parameter
  ...
  running a code snippet and examining the output, so providing an instant feedback is essential.
\end{itemize}
...
\begin{figure}[b]
  ...
  \subcaptionbox{
    \raggedright\footnotesize
      Our live coding environment for The Gamma language based on the theory presented in this paper.
      The table is produced as
      the data analyst types and updates on-the-fly to show result at the current cursor position.
    \label{fig:gamma-preview}
  }{\includegraphics[scale=0.26]{gamma2.png}}
  \caption{\small Previous work on The Gamma by \citet{gamma} (left) and our new editor with live preview (right).}
...
Data exploration of the kind discussed in the previous section has been the motivation for
recent work by \citet{gamma} that develops a small scripting language called The Gamma.
Scripts in The Gamma are sequences of commands that can either define a variable or produce an
output. The language does not support top-level function declarations
...
such as JavaScript. The Gamma supports type providers \cite{providers-fsharp},
which generate types on the fly, for accessing and exploring external data sources.
...
possible to complete a large number of data exploration tasks through the very simple interaction of
selecting operations from a list. An example in Figure~\ref{fig:gamma-dot} summarizes data on
Olympic medals using a type provider. Identifiers such as \qident{sum Bronze}
are merely names of members generated by the type provider, based on information
about the data source. The type provider used in this example generates an object with members for
different data transformations, such as \qident{group data}, which return further objects with
...
When we refer to The Gamma in this paper, readers not familiar with it can consider a subset of Python.
...
language. The implementation is discussed in Section~\ref{sec:evaluation-case} and it replaces
the text editor with auto-complete, originally used by The Gamma, with a live coding environment
that provides \emph{live previews}. We choose to extend The Gamma over using a subset of Python
for its simplicity as it lets us focus on the principles of live coding systems.
...
The \emph{data exploration calculus} is a small formal language for data exploration.
The calculus is not, in itself, Turing-complete and it can only be used together with external libraries that
...
define the calculus in this section and then use it as the basis for discussing our live preview
...
Interestingly, the mechanism that is used for live previews can also be used for instant type checking.
...
additive and the preview can be updated by evaluating just the newly added
...
We write $\Delta(\bkndclr{k}, l)\!\downarrow$ when a value for a given key is not defined.
...
symbol $s_2$ and later insert it into the cache.
...
dependency graph after program is updated and parsed. During live coding, this is done
...
previews for expressions (or sub-expressions) of a program are reused when updating a preview.
...
would get by directly evaluating the code and, equally importantly, we show that no recomputation
occurs when code is edited in certain ways.
...
to the lambda function and display a preview based on those. In Section~\ref{sec:extra-abstraction}
we also consider a more speculative design for an abstraction mechanism that supports live previews,
which could, in some cases, replace lambda function.
...
evaluated using~$\Downarrow$ using the corresponding graph node as a lookup key.
...
\noindent
The mechanism used for caching previews (Section~\ref{sec:previews-cache}),
keeps a preview preview $d$ in a lookup table indexed by nodes $\bndclr{v}$.
Theorem~\ref{thm:determinacy} guarantees that this is a valid -- as we update dependency
...
that have the same value and will not need to be recomputed after the change; we write
$\overline{c}$ and $\overline{e}$ for a list of commands and expressions, respectively.
In some of the edit operations, we also specify an intermediate program that may be
...
\rname{let-intro-ins}, we the analyst cuts a sub-expression $e$, replaces it with
...
binding, several ways of eliminating a let binding and two edit operations where the data analyst
...
\noindent
\hspace{-1em}
\includegraphics[scale=0.245]{gallery1.png}
\includegraphics[scale=0.245]{gallery2.png}
\caption{\small Data analysis counting the number of libraries per county in the UK. We load
  ...
  The example is adapted from \url{http://gallery.thegamma.net/73/}.}
\label{fig:gallery}
\vspace{-1em}
\end{figure}
...
% \section{Appendix - Type checking}
% \label{sec:app-types}
%
% tbd
