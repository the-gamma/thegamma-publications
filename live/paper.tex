...
We need to cache type checking results too.

The key trick is to separate the process into faster binding which constructs dependency
graph and slower type checking and evaluation. (This assumes that binding can be done, perhaps
as an over-approximation without full type checking.)
...
(Theorem~\ref{thm:correcntess}) shows that, no matter how a graph is constructed, when we use
it to evaluate a preview for an expression, the preview is the same as the value we would obtain
by evaluating the expression directly. Determinacy (Theorem~\ref{thm:determinacy}) shows that if
...
\subparagraph{1. Introducing let}

$C_1[C_2[e]]$ to
$C_1[\kvd{let}~x=e~\kvd{in}~C_2[x]]$

Also holds for multiple replaces. 

\subparagraph{2. Eliminating let}

$C_1[\kvd{let}~x=e~\kvd{in}~C_2[x]]$ to
$C_1[\kvd{let}~x=e~\kvd{in}~C_2[e]]$ to
$C_1[C_2[e]]$ 
...
\subparagraph{3. Editing non-dependency in let}

$C[\kvd{let}~x=e_1~\kvd{in}~e_2]$ such that $x\notin FV(e_2)$ to
$C[\kvd{let}~x=e_1'~\kvd{in}~e_2]$

\subparagraph{3. Editing non-dependency in chain} a

$C[e.m(e_1, \ldots, e_n).m'(e'_1, \ldots, e'_k)]$ to

$C[e.m(e_1, \ldots, e_n).m''(e''_1, \ldots, e''_k)]$ 

preview of $e.m(e_1, \ldots, e_n)$ remains
