...
a text editor. This is challenging as the user can make arbitrary edits and we cannot fully 
recompute previews after every change. The presented interpreter partially evalautes programs as 
they are written and reuses results of previous evaluations, making it possible to provide instant 
...
ordinary text editor. The main contributions of this paper are:
...
\item In the final more speculative part (Section~\ref{sec:design}), we consider alternative
...
\begin{lem}[Let-free correctness]
Given an expression $e$ that has no free variables and does not contain let bindnigs, together with
a lookup table $\Delta$ obtained from any sequence of expressions according to Figure~\ref{fig:loop}, 
...
\end{lem}
\begin{proof}
Every dependency graph node $\bndclr{v}$ corresponds to an expression $e$ with free variables
$FV(e)$. We obtain preview $\bndclr{v}\Downarrow d$ such that if $FV(e) = \emptyset$ then
$d = p$ for some $p$, otherwise, $d = \llbracket e' \rrbracket_{FV(e)}$.  
We show that either $e \rightsquigarrow p$ or that for any $C$ such that $FV(C[e]) = \emptyset$
it holds that $C[e] \rightsquigarrow e''$ and $C[e'] \rightsquigarrow e''$.
By induction over the structure of the evaluation:

\vspace{0.5em}\noindent\emph{(val)} $\bnd{val}(n) \Downarrow n$ and so $p=n$. The only
  \ident{bind} case that generates $\bnd{val}(n)$ in the absence of let bindings is~(4) and
  so $e = n$.

\vspace{0.5em}\noindent\emph{(mem-val)} $\bnd{mem}(m, s) \Downarrow p$. By induction, there
  are nodes $\bndclr{v_i}$ representing expressions $e_i$ and previews $\bndclr{v_i}\Downarrow p_i$
  such that $e_i \rightsquigarrow p_i$. The only \ident{bind} case that generates the $\bnd{mem}(m, s)$
  node in the absence of let binding is~(2) and so $e=e_0.m(e_1, \ldots, e_n)$. From 
  (\emph{mem-val}), $p_0.m(p_1, \ldots, p_n) \rightsquigarrow p$ and from the compositionality 
  of $\rightsquigarrow$, it is the case that $e \rightsquigarrow p$.
