...
\\
\inference[(fun-val)~]
  { (\bnd{fun}(x,s), \bndclr{v_b}, \blbl{body}) \in E \\
    (\bnd{var}(x,s), \bndclr{v_c}, \blbl{callsite}(m, i)) \in E \\
...
\begin{equation*}
\begin{array}{lcll}
\bkndclr{k}&\in&\bknd{fun}(x)~|~\bknd{mem}(m)~|~\bknd{var}(x)&\quad(\textit{Node kinds})\\
c&\in&\bot ~|~(\bndclr{v}, \blblclr{l})&\quad(\textit{Call sites})\\
\end{array}
\end{equation*}
%
...
Update Figure~\ref{fig:loop} accordingly

We show cases (2) and (7); cases (1) and (6) that reuse nodes are analogous;
other cases just call bind recursively with no node
