...
that type providers are integrated into the type system and types can be provided lazilly. This is 
...
\item The filtering operation is written using indexing $[\ldots]$ while all other operations are
  ...
  while in the other, we specify a column name using $\ident{by}=\str{Gold}$. In other languges,
...
on the values in the data source (we discuss this furter in Section~\ref{sec:pivot-filter}).
...
the language, but the ``then'' pattern lets us express nesting without langauge support. In both 
...
provide information about types on demand. In our formalisation, we follow the 
...
In the revised formalisation, the \ident{pivot} function which models the type provider takes the schema
...
of gold medals appear first, but silver or bronze medals are not yed used as secondary sorting keys.
...
formalise the type provider and prove that queries constructed using the types it provides are 
correct. Achieving this property by ohter means would require a language with complex
...
in the examples disussed throughout the paper is a single CSV file listing the entire history of
