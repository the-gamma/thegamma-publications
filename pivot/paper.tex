...
between code and user interface. This is done through a direct manipulation interface [X] inspired
by spreadsheet applications. When exploring data in a spreadsheet, the user can always see the 
...
Unlike the user interface for specifying lists, the live editor has been implemented specifically
for the pivot type provider, but it still heavily relies on the simple structure provided by the 
fact that entire transformation can be written as a single chain of member accesses. In particular,
we identify individual transformations (\qident{group by}, \qident{sort by}, etc.) and then 
generate different user interface for specifying parameters. For sorting, as shown in 
Figure~\ref{fig:case-ed}, the second level lets the user add or remove members. For grouping or
paging, the second level looks different and lets the user choose the key and the number of 
...
https://people.csail.mit.edu/ebakke/research/sieuferd_sigmod2016.pdf
