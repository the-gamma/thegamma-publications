...
include the ``You draw It'' visualization introduced by the New York Times. % cite
...
interactivity are supported by high-level libraries. Google Charts supports panning using
drag \& drop, zooming to a selected chart range and animations. Custom interactivity can be
...
\qquad \qquad \fkvd{for}~\ident{party}, \ident{mps}~\fkvd{in}~s~\rightarrow~\kvd{padding}~0,10,0,10,~\kvd{fill}~(\ident{color}~\ident{party}),\\
