...
open Compost.Html
open Compost
open Compost.Derived
open Compost.Html.Common
...
module InteractiveHelpers =
  let showAppAsync outputId opts (data:(_ * _)[]) initial render update = async { 
    ...
    if data.Length = 0 && opts.emptyMessage.IsSome then
      ...
      do
        ...
        with e ->
          failwithf "Interactive rendering failed: %O" e } 
...
type YouGuessLine = 
  ...
  member y.show(outputId) = Async.StartImmediate <| async {
    let markers = defaultArg y.markers [||]
...
type CompostLinesAreasChart =
  ...
  member y.show(outputId) = Async.StartImmediate <| async {
    let data = y.data
    let allData = Array.concat y.data 
    return! InteractiveHelpers.showStaticAppAsync outputId y.options allData
      (fun size -> 
        let lcs = defaultArg y.lineColors vega10
        Charts.renderLines y.isArea y.options size lcs (y.data |> Seq.map (fun s -> "series")) data) }
