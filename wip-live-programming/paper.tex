...
them interactively based on the results. We aim to build foundations for such seamless integration 
...
initial focus is on simple use cases, such as those of journalists, because we believe that this 
is where better tooling can provide the most value.
...
\begin{figure}
\includegraphics[scale=0.28]{gamma1.png}
\caption{\small The Gamma script to aggregate Olympic medals. We select Rio 2016 games and
...
\begin{figure}
\includegraphics[scale=0.28 ]{gamma2.png}
\caption{\small Live preview in The Gamma programming environment. The table is produced as
...
\paragraph{Text editor integration.}
...
\paragraph{Data stay, code changes.}
...
\quad
\begin{array}{lcl}
a &=& e \lsep \lambda x\rightarrow e\\
v &=& o \lsep \lambda x\rightarrow e\\
\end{array}
\end{equation*}
...
\hspace{-2em}  
\begin{array}{lcl}
C_e[-] &=& -.m(a_1, \ldots, a_n) ~|~ -.m(v_1, \ldots, v_{m}, -, a_1,\ldots , a_n) ~|~ -\\
C_c[-] &=& \kvd{let}~x = C_e[-] ~|~ C_e[-]\\
C_p[-] &=& c_1;\; \ldots;\; c_{k-1};\; C_c[-];\; c_{k+1};\; \ldots;\; c_n\\
\end{array}
\end{equation*}
...
c_1;\; \ldots;\; c_{k-1};\; \kvd{let}~x=o;\; c_{k+1};\; \ldots; c_n \rightsquigarrow\\
\qquad  c_1;\; \ldots;\; c_{k-1};\; c_{k+1}[x\leftarrow o];\; \ldots; c_n[x\leftarrow o]\end{array} &
...
such as lambda functions. The syntax is defined in Figure~\ref{fig:dec-calculus}. We write $x$ for 
variables and $o$ for object values. A program $p$ in the data exploration calculus consists of a 
sequence of commands $c$. A command can be either let binding or an expression. Lambda functions 
can only appear directly as arguments in method calls. An argument $a$ can thus be a lambda 
...
objects $o_1;\; \ldots;\; o_n$ which can be displayed as results of the data analysis. The 
