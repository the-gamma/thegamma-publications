...
This paper focuses on simple data wrangling and data exploration as done, for example, by 
journalists analysing government datasets. This section illustrates how such data analyses 
look and provides a justification for design decisions that we make when designing the data 
exploration calculus.
...
Increasing numbers of data analysts use notebook systems such as Jupyter or R Markdown, which 
make it possible to combine text, equations and code with results of running the code such as tables 
or visualizations. Notebooks blur the traditional distinction between development and execution. 
Data analysts write small snippets of code, run them immediately to see results and then revise 
them interactively based on the results. We aim to take this trend further and build foundations
for seamlessly integrating development and execution in a notebook-like environment.
...
focus is on simple use cases, such as those of journalists, because we believe that this is where
better tooling can provide the most value.

As an example, consider the data anlysis done by the Financial Times for an article on plastic 
waste. The analysis joins datasets from Eurostat, UN Comtrade and more, aggregates the data and 
builds a visualization comparing the plastic waste flows in 2017 and 2018.
...
\item There is no abstraction. The analysis uses lambda functions as arguments to library calls,
  but it does not define any functions. Parameterization is done either by defining a list of 
...
important problem for programming tools research.

\begin{figure}
\includegraphics[scale=0.6]{notebook.png}
...
\begin{figure}
\includegraphics[scale=0.52]{gamma.png}
\caption{The Gamma script to aggregate waste data. We select three countries and group
data by country. We then count the number of distinct reporting periods per country and type `.'
to choose further aggregation operations.}
\label{fig:gamma-waste}
\vspace{-1em}
\end{figure}
...
for a recent programming language research project The Gamma. The scripting language in The Gamma
...
Given the expressiveness of The Gamma, libraries for it are implemented in other languages, such as
...
a list. An example in Figure~\ref{fig:gamma-waste} summarizes waste data using a type provider.
The names of identifiers such as \qident{average Waste (kg)} do not have
