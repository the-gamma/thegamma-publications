...
can even export the workflow as a script \cite{wrangler}, but data analysts who prefer working with
...
Gamma program consist of a list of commands which are either expressions or let bindings. Our
...
appealing alternative and make recomputation easier, we prefer the transparency
...
$C_c$ specifies evaluation of a command and $C_p$ defines top-to-bottom evaluation of a program.
...
terms, the context-dependent proposition can be modelled as an indexed comonad \cite{effectrev,graded}.
...
Gamma language that provides instantaneous feedback during coding. The environment uses the
...
with an abstraction mechanism that would support development with instantaneous feedback is an
...
provide instant feedback. We aim to make programming tools as instantaneous as spreadsheets.
